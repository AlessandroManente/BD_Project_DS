...
def swiss_prot_parser():
    if 'uniprot_sprot.csv' not in os.listdir(
            path.join('data_team_1', 'swiss_prot')):
        ...
        swissprot_df.to_csv(cur + '\\data_team_1\\swiss_prot\\' +
                            'uniprot_sprot.csv')

    else:
        swissprot_df = pd.read_csv(
            path.join('data_team_1', 'swiss_prot', 'uniprot_sprot.csv'))
...
def parse_psiblast():
    dirs_to_parse = [
        path.join('data_team_1', 'PSSMs', 'PSSM_{}'.format(a), 'to_parse')
        for a in ['C', 'M', 'O']
    ]
    #dirs_to_parse = [cur + '\\data_team_1\\PSSMs\\PSSM_' + a + '\\to_parse' for
    #a in ['C', 'M', 'O']]
    dirs_parsed = [
        path.join('data_team_1', 'PSSMs', 'PSSM_{}'.format(a), 'parsed')
        for a in ['C', 'M', 'O']
    ]
...
def metrics_8(gt,
    ...
    if 'metrics_8_{0}_{1}_{2}.csv'.format(
            h_threshold_name, hi_threshold_name,
            p_threshold_name) in os.listdir(path.join('data_team_1',
                                                      'metrics')):
...
def metrics_9(parsed_domtblouts,
    ...
    # p_threshold_name) in os.listdir(cur + '\\data_team_1\\metrics'):
    if 'metrics_9_{0}_{1}_{2}.csv'.format(
            h_threshold_name, hi_threshold_name,
            p_threshold_name) in os.listdir(path.join('data_team_1',
                                                      'metrics')):
        # old_metrics_df = pd.read_csv(cur +
...
def plot_metrics_summary(metrics_df,
    ...
    plt.savefig(
        path.join(
            'data_team_1', 'metrics', 'best_models_{0}_{1}_{2}_{3}.png'.format(
