...
def compute_con_matrix_9(parsed_domtblouts, parsed_psiblast, gt):
    ...
    for k, (df_name, df_dict) in enumerate(dict_dfs.items()):
        gt_int_df = [x for x in gt_acc if x in list(df_dict.keys())] # ids both in gt and df
        df_not_gt = [x for x in list(df_dict.keys()) if x not in gt_acc] # ids in df and not in gt
        gt_not_df = [x for x in gt_acc if x not in list(df_dict.keys())] # ids in gt and not in df
